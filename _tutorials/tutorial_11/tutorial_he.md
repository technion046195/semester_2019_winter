...
2. $$\alpha_i\geq0$$ and $$y_i\left(\boldsymbol{w}_i^Tx_i+b\right)=1$$

<br>

לרוב בפתרון של בעיות אלו נקבל שרק חלק קטן מן הנקודות יקיימו $$y_i\left(\boldsymbol{w}_i^Tx_i+b\right)=1$$. נקודות אלו 
נקראות וקטורי התמיכה (support vectors) של הבעיה.

מכיוון שרק בעבור נקדות אלו יכול להתקיים $$\alpha_i>0$$ ומתוך על פי המשוואה $$\boldsymbol{w} =\sum_i\alpha_iy_i\boldsymbol{x}_i$$, ניתן להסיק שרק דוגמאות אלו משתתפת בהגדרת הערך של $$w$$.

<br>

### תכונות ה- Support Vectors

לסיכום: נקודות המקיימות $$y_i\left(\boldsymbol{w}_i^Tx_i+b\right)=1$$ נקראות וקטורי התמיכה (support vectors), הם מקיימות את התכונות הבאות:

1. $$\alpha_i>0$$ רק אם $${{x}_{i}}$$ היא SV. (הכיוון ההפוך לא בהכרח מתקיים, כלומר יכולה להיות דגימה $${{x}_{j}}$$ שהיא SV, אך $$\alpha_j=0$$).

2. מרחקן למישור המפריד הוא מינימלי. המרחק האוקלידי של ה SV מהמישור המפריד נקרא ה- margin של הבעיה, והוא שווה ל $$\frac{1}{||w||}$$ 
   (**תרגיל**: הוכיחו את הביטוי הנ"ל, בעזרת הביטוי למרחקוההגדרה של ה SV).
